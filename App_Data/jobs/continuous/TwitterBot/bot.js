...
(function() {
	...
	var config = {
		...
		"follow_user_ids_csv": process.env['follow_user_ids_csv']
	};

	//create an object using the keys we just determined
	var twitterAPI = new ntwitter(config.keys.twitter);
		...
		stream.on('data', function(data) {
			...
			if(data.text !== undefined) {
				...
				//	&& data.retweeted_status === undefined) {										//and if it isn't a retweet of one of our tweets
					...
					LogUtils.logtrace("[" + data.id_str + "] [" + data.user.screen_name + "]: " + data.text , LogUtils.Colors.GREEN);
	...
	function initStreaming() {
		...
		twitterAPI.stream('statuses/filter', { 'follow': config.follow_user_ids_csv}, streamCallback);
	}
